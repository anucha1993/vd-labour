        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">แก้ไขงาน: {{ $job->job_number }}</h4>
                    <div>
                        <a href="{{ route('jobs.show', $job->job_id) }}" class="btn btn-info">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('jobs.update', $job->job_id) }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="job_number" class="form-label">หมายเลขงาน</label>
                                    ...
                                    <select class="form-select @error('country_id') is-invalid @enderror" 
                                        ...
                                        @foreach($countries as $country)
                                            <option value="{{ $country->country_id }}" 
                                                    {{ old('country_id', $job->country_id) == $country->country_id ? 'selected' : '' }}>
                                    ...
                                    <select class="form-select @error('dm_id') is-invalid @enderror" 
                                        ...
                                        @foreach($demands as $demand)
                                            <option value="{{ $demand->dm_id }}" 
                                                    {{ old('dm_id', $job->dm_id) == $demand->dm_id ? 'selected' : '' }}
                         ...
                         <div class="col-md-3">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('job_total') is-invalid @enderror" 
                                           id="job_total" name="job_total" value="{{ old('job_total', $job->job_total) }}" 
                                           min="1" required>
                                    @error('job_total')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <div class="form-text">
                                        ปัจจุบัน: ได้งานแล้ว {{ $job->accepted_count }} คน | เหลือ {{ $job->remaining_positions }} คน
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('job_status') is-invalid @enderror" 
                                        ...
                                        <option value="เปิดรับสมัคร" {{ old('job_status', $job->job_status) == 'เปิดรับสมัคร' ? 'selected' : '' }}>เปิดรับสมัคร</option>
                                        <option value="ปิดรับสมัคร" {{ old('job_status', $job->job_status) == 'ปิดรับสมัคร' ? 'selected' : '' }}>ปิดรับสมัคร</option>
                                    </select>
                                    @error('job_status')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
